require(['modules/jquery-mozu', 'hyprlive',"modules/backbone-mozu",  "modules/models-product", "modules/api", 'modules/models-orders'],
    function ($, Hypr, Backbone, ProductModels, Api, OrderModels) {

    	$(document).ready(function () {
            
            var res=Api.get('entityList',{listName: 'bvsettings@mzint', id: Api.context.site });
            res.then(function(r) { 
                ...
                $.getScript(bvScript).done(function( script, textStatus ) {
                    if (isWidget) {
                        var currentProduct = ProductModels.Product.fromCurrent();
                        $BV.configure('global', { productId : currentProduct.id});
